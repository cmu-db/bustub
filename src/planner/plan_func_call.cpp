...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
  ...
  if (func_name != "lower" && func_name != "upper") {
    throw Exception(fmt::format("Unknown func call {}", func_name));
  }
  if (args.size() != 1) {
    throw Exception(fmt::format("func call {} args should be 1", func_name));
  }
  StringExpressionType type = func_name == "lower" ? StringExpressionType::Lower : StringExpressionType::Upper;
  auto ptr = new StringExpression(args[0], type);
  return AbstractExpressionRef(ptr);
}
